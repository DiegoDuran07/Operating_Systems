...
MAX_BLOCKED_TIME=6  # Changed from 7 to 6 as per requirements
WINDOW_HEIGHT=750
WINDOW_WIDTH=1000  # Increased width for better layout
FRAME_LENGTH=5
FRAME_AMOUNT=48
OPERATIVE_SISTEM_MEM_SIZE=4
SUSPENDED_FILE = "suspended_processes.json"
...
class Process():
    ...
    def to_dict(self):
        ...
        # We exclude the occupied frames as they can't be easily serialized
        return {
            'id': self.id,
            'state': self.state,
            ...
            'finalizationTime': self.finalizationTime,
            'returnTime': self.returnTime,
            'waitTime': self.waitTime,
            'responseTime': self.responseTime,
            'quantumTime': self.quantumTime,
            'hasBeenExecuted': self.hasBeenExecuted,
    ...
    def from_dict(cls, data):
        ...
        process.elapsedTime = data['elapsedTime']
        process.remainingTime = data['remainingTime']
        process.blockedTime = data['blockedTime']
        process.arrivalTime = data['arrivalTime']
        process.finalizationTime = data['finalizationTime']
        process.returnTime = data['returnTime']
        process.waitTime = data['waitTime']
        process.responseTime = data['responseTime']
        process.quantumTime = data['quantumTime']
        process.hasBeenExecuted = data['hasBeenExecuted']
        return process
...
class RR():
    ...
    def __init__(self):
        ...
        self.suspendedProcesses = []  # New suspended processes list
    ...
    def processingWindow(self):
        ...
        self.processingWindow.bind("s", self.sPressed)  # New binding for Suspend key
        self.processingWindow.bind("r", self.rPressed)  # New binding for Return key
        ...
        self.suspendedProcesses=[]  # Initialize suspended processes list as empty

        # We don't load suspended processes from file - always starting with 0 suspended processes
        # This is the modification requested by the user
        ...
        quantumValueLabel=ttk.Label(quantumValueFrame, 
                                    text=f'Quantum: {self.quantum} | Contador: 0', 
        ...
        suspendedThread=threading.Thread(target=self.suspendedProcessesManagement)  # New suspended thread

        timerThread.daemon = True
        newThread.daemon = True
        readyThread.daemon = True
        blockedThread.daemon = True
        executionThread.daemon = True
        finishedThread.daemon = True
        suspendedThread.daemon = True
    ...
    def createProcessStateFrames(self):
        ...
        # Frame for suspended processes (new)
        suspended_frame = ttk.LabelFrame(self.processingWindow, text="Procesos Suspendidos", padding=10)
        suspended_frame.grid(row=0, column=2, rowspan=1, padx=5, pady=60, sticky="nsew")
        self.suspended_text = tk.Text(suspended_frame, height=10, width=40, state='disabled')
        self.suspended_text.pack(fill='both', expand=True)

        # Frame for finished processes
        finished_frame = ttk.LabelFrame(self.processingWindow, text="Procesos Terminados", padding=10)
        finished_frame.grid(row=1, column=2, rowspan=1, padx=5, pady=5, sticky="nsew")
        self.finished_text = tk.Text(finished_frame, height=10, width=40, state='disabled')
    ...
    def timer(self, label):
        ...
        self.generalTime=1
        while self.areProcessesRunning():
            # Checking if paused
        ...
        self.saveSuspendedProcesses()  # Save suspended processes before exiting
        self.showBcp()
                
    # Check if there are still processes running in any queue
    def areProcessesRunning(self):
        return (len(self.newProcesses) > 0 or 
                len(self.readyProcesses) > 0 or 
                len(self.executionProcess) > 0 or 
                len(self.blockedProcesses) > 0 or 
                len(self.suspendedProcesses) > 0)
    ...
    # Function to manage suspended processes
    def suspendedProcessesManagement(self):
        while True:
            # Update suspended processes text
            self.suspended_text.config(state='normal')
            self.suspended_text.delete(1.0, tk.END)
            
            if len(self.suspendedProcesses) > 0:
                next_process = self.suspendedProcesses[0]
                text = f"Cantidad: {len(self.suspendedProcesses)}\n\n"
                text += f"Próximo a regresar:\nID: {next_process.id}\nTamaño: {next_process.memSize}\n"
            else:
                text = "Cantidad: 0\n\nNo hay procesos suspendidos"
                
            self.suspended_text.insert(tk.END, text)
            self.suspended_text.config(state='disabled')
            time.sleep(TIME_STEP_SECONDS/2)
...
# Check if quantum time is completed for the process
                elif len(self.executionProcess) and self.executionProcess[0].quantumTime >= self.quantum:
                    # Memory management
                    for i in range(len(self.executionProcess[0].ocuppiedFrames)):
                ...
                # Check if ready deque has processes, if so, it assigns the first ready to execution
                elif len(self.readyProcesses) and not len(self.executionProcess):
                    # Memory management
                    for i in range(len(self.readyProcesses[0].ocuppiedFrames)):
                        self.readyProcesses[0].ocuppiedFrames[i].setColor(Colors.EXECUTING.value)
                    # Deque management
                    self.readyProcesses[0].state = 'EJECUTANDO'
                    if not self.readyProcesses[0].hasBeenExecuted:
                        self.readyProcesses[0].hasBeenExecuted = True
                        self.readyProcesses[0].responseTime = self.generalTime - self.readyProcesses[0].arrivalTime
                    self.executionProcess.append(self.readyProcesses.popleft())

                # If theres an execution process, it increments elapsed time
                if len(self.executionProcess):
    ...
    # Function to manage finished processes deque behavior
    ...
    # Function to save suspended processes to file
    def saveSuspendedProcesses(self):
        if len(self.suspendedProcesses) > 0:
            suspended_data = [process.to_dict() for process in self.suspendedProcesses]
            with open(SUSPENDED_FILE, 'w') as file:
                json.dump(suspended_data, file)

    # Function to load suspended processes from file
    def loadSuspendedProcesses(self):
        try:
            if os.path.exists(SUSPENDED_FILE):
                with open(SUSPENDED_FILE, 'r') as file:
                    suspended_data = json.load(file)
                    self.suspendedProcesses = [Process.from_dict(data) for data in suspended_data]
        except Exception as e:
            print(f"Error loading suspended processes: {e}")

    # Function to show BCP window
    def showBcp(self):
        ...
        # Window setup
        self.bcpWindow = tk.Toplevel(self.processingWindow)
        self.bcpWindow.title('Table of Process Control Blocks')
        self.bcpWindow.geometry('1200x600')
        self.bcpWindow.resizable(True, True)

        # Creating the table
        columns = ('ID', 'Estado', 'Operación', 'Resultado', 'TME', 'TT', 'TR', 'TL', 'TF', 'TRET', 'TE', 'TS', 'Tamaño')
        table = ttk.Treeview(self.bcpWindow, columns=columns, show='headings')
        ...
        all_processes = (
            list(self.newProcesses) + 
            list(self.readyProcesses) + 
            list(self.executionProcess) + 
            list(self.blockedProcesses) + 
            list(self.suspendedProcesses) + 
            list(self.finishedProcesses)
        )
        ...
        for process in all_processes:
            table.insert('', tk.END, values=(
                ...
                process.state, 
                process.operation, 
                process.result if process.state == 'TERMINADO' else 'N/A', 
                process.serviceTime,
                process.elapsedTime,
                process.remainingTime,
                process.arrivalTime,
                process.finalizationTime if process.state == 'TERMINADO' else 'N/A',
                process.finalizationTime - process.arrivalTime if process.state == 'TERMINADO' else 'N/A',
                (process.finalizationTime - process.arrivalTime - process.serviceTime) if process.state == 'TERMINADO' else 'N/A',
                process.responseTime,
                process.memSize
            ))

        # Add scrollbar
        scrollbar = ttk.Scrollbar(self.bcpWindow, orient=tk.VERTICAL, command=table.yview)
        ...
        self.bcpWindow.protocol("WM_DELETE_WINDOW", on_closing)

    # Function to show page table window
    def showPageTable(self):
        if self.pageShowing.is_set():
            return
        self.pageShowing.set()

        # Window setup
        self.pageWindow = tk.Toplevel(self.processingWindow)
        self.pageWindow.title('Table of Pages')
        self.pageWindow.geometry('600x600')
        self.pageWindow.resizable(True, True)

        # Creating the table
        columns = ('Marco', 'Estado', 'Proceso')
        table = ttk.Treeview(self.pageWindow, columns=columns, show='headings')
        ...
        for i, frame in enumerate(self.memory.frames):
            estado = "Sistema Operativo" if i < OPERATIVE_SISTEM_MEM_SIZE else (
                "Libre" if frame.isEmpty() else "Ocupado"
            )
            
            proceso = "SO" if i < OPERATIVE_SISTEM_MEM_SIZE else "-"
            if not frame.isEmpty() and i >= OPERATIVE_SISTEM_MEM_SIZE:
                # Find which process is using this frame
                for process in (list(self.readyProcesses) + list(self.executionProcess) + list(self.blockedProcesses)):
                    if frame in process.ocuppiedFrames:
                        proceso = f"ID: {process.id}"
                        break
            
            table.insert('', tk.END, values=(i, estado, proceso))

        # Add scrollbar
        scrollbar = ttk.Scrollbar(self.pageWindow, orient=tk.VERTICAL, command=table.yview)
        table.configure(yscroll=scrollbar.set)
        ...
        self.pageWindow.protocol("WM_DELETE_WINDOW", on_closing)

    # Event handlers for keyboard presses
    ...
    # S event handler (new for suspend)
    def sPressed(self, event):
        if len(self.blockedProcesses) > 0:
            # Get the first blocked process
            process = self.blockedProcesses.popleft()
            
            # Free up memory frames
            for frame in process.ocuppiedFrames:
                frame.setColor(Colors.EMPTY.value)
                frame.setOcuppiedCells(0)
            
            # Clear occupied frames reference since it's no longer in memory
            process.ocuppiedFrames = []
            process.state = 'SUSPENDIDO'
            
            # Add to suspended processes
            self.suspendedProcesses.append(process)
            
            # Save the updated suspended processes list
            self.saveSuspendedProcesses()

    # R event handler (new for return suspended process)
    def rPressed(self, event):
        if len(self.suspendedProcesses) > 0:
            # Get the first suspended process
            process = self.suspendedProcesses.pop(0)
            
            # Try to find memory space for it
            framesReserved = []
            neededFrames = 0
            processMemSize = process.memSize
            ...
            for frame in self.memory.frames:
                if frame.isEmpty():
                    freeFrames += 1
                    framesReserved.append(frame)
                if freeFrames == neededFrames:
                    # We found enough frames, set them up
                    # Calculate last frame occupancy
                    nonFrameLenghtMultipleFrames = ((processMemSize*-1) - FRAME_LENGTH)*-1
                    
                    if nonFrameLenghtMultipleFrames < 5:
                        for k in range(len(framesReserved) - 1):
                            framesReserved[k].setOcuppiedCells(FRAME_LENGTH)
                        framesReserved[-1].setOcuppiedCells(nonFrameLenghtMultipleFrames)
                    else:
                        for frame in framesReserved:
                            frame.setOcuppiedCells(FRAME_LENGTH)
                    
                    # Color the frames
                    for frame in framesReserved:
                        frame.setColor(Colors.READY.value)
                    
                    # Update process and add to ready queue
                    process.ocuppiedFrames = framesReserved
                    process.state = 'LISTO'
                    process.blockedTime = 0
                    self.readyProcesses.append(process)
                    return
            
            # If we couldn't find enough memory, put it back in suspended queue
            self.suspendedProcesses.insert(0, process)
            messagebox.showinfo("Memoria insuficiente", 
                                "No hay suficiente memoria para restaurar el proceso suspendido.")
